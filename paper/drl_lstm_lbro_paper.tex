...
The exponential growth of Internet of Things (IoT) devices has placed
unprecedented demand on computing infrastructure. Traditionally,
compute-intensive IoT tasks were offloaded entirely to centralised
cloud data centres over the wide-area network. While cloud computing
offers virtually unlimited resources, this approach introduces
round-trip latencies in the range of 100--300\,ms, which are
unacceptable for latency-critical applications such as autonomous
vehicles, industrial automation, augmented reality, and real-time
health monitoring. Furthermore, transmitting raw sensor data from
thousands of devices to a distant cloud generates substantial
backhaul congestion and raises privacy concerns. These limitations
drove the shift toward Mobile Edge Computing (MEC)~\cite{shi2016edge},
which relocates compute capacity to cloudlets at the network edge,
reducing propagation delay to single-digit milliseconds while
...
\begin{enumerate}
    ...
    explicitly address through multi-component penalty design~\cite{mao2016resource,liu2022edgelb}.
...
degrades significantly under congestion (2.76\% drops in our
...
achieves 2.00\% drops—better than LBRO but $3.4\times$ worse than our
approach.
...
stateless greedy assignment, yielding 4.35\% drops in our heterogeneous
...
is approximated at inference time. Their approach achieves 2.07\% drops
in our testbed, comparable to DeepRM but still $3.6\times$ worse than
...
at each slot. This Pred-LB approach records 5.24\% drops in our
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{fig_system_model.png}
\caption{Three-tier IoT--Edge--Cloud system model.
The task broker (DRL-LSTM-LBRO) routes each IoT task to one of three
heterogeneous cloudlets or the remote cloud. Architecture adapted from
the MEC deployment model described in~\cite{shi2016edge}.}
\label{fig:sysmodel}
\end{figure}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{fig_architecture1.png}
\caption{DRL-LSTM-LBRO two-stage pipeline. Stage~1: per-cloudlet GRU-LSTM predictors
forecast CPU and RAM utilisations five slots ahead; outputs feed directly into the
23-dimensional state vector. Stage~2: DDQN agent selects task placement
($a \in \{c_0, c_1, c_2, \text{cloud}\}$) and updates via experience replay.}
\label{fig:arch}
\end{figure*}
...
\hat{u}_{\text{cpu},i},\, \hat{u}_{\text{ram},i},\, \mathbf{1}[\text{crit}_i]]$
...
\begin{equation}
r_t = -W_l \hat{l}_t - W_d \mathbf{1}[\text{drop}] -
...
\midrule
$N$                          & Number of IoT devices (50) \\
$\lambda$                    & Task arrival rate per slot \\
$c_i$                        & Number of servers in cloudlet $i$ \\
$\mu_i$                      & Service rate of cloudlet $i$ (MIPS) \\
$\rho_i$                     & Traffic intensity at cloudlet $i$ \\
$K_i$                        & Queue capacity of cloudlet $i$ \\
$u_{\text{cpu},i}$           & CPU utilisation of cloudlet $i$ \\
$u_{\text{ram},i}$           & RAM utilisation of cloudlet $i$ \\
$u_{\text{q},i}$             & Queue fill ratio of cloudlet $i$ \\
$\hat{u}_{\text{cpu},i}$     & LSTM-predicted CPU utilisation \\
$\hat{u}_{\text{ram},i}$     & LSTM-predicted RAM utilisation \\
$\mathbf{1}[\cdot]$          & Indicator function (1 if condition true, else 0) \\
$d_{\text{cpu}},d_{\text{ram}}$ & Task CPU demand (MIPS) and RAM demand (MB) \\
$\phi_i$                     & Dynamic task priority score~\cite{aicdqn2026} \\
$\mathbf{s} \in \mathbb{R}^{23}$ & DDQN state vector \\
$a \in \{0,1,2,3\}$          & Placement action ($c_0,c_1,c_2$, cloud) \\
$r_t$                        & Reward at step $t$ (Eq.~\ref{eq:reward}) \\
$T_t$                        & End-to-end task latency at step $t$ \\
$T_{\max}$                   & Maximum latency (normalisation constant) \\
$\hat{l}_t = T_t/T_{\max}$   & Normalised latency \\
$\sigma(\mathbf{u})$         & Std.\ deviation of cloudlet CPU utilisations \\
$W_l, W_d, W_i, W_o$        & Reward weights: latency, drop, imbalance, overload \\
$\gamma$                     & Discount factor \\
$\varepsilon$                & Exploration rate ($\varepsilon$-greedy) \\
$\theta,\,\theta^{-}$        & Online and target DDQN network parameters \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{fig3_baseline_comparison.png}}
...
DRL-LSTM-LBRO achieves the lowest drop rate of 0.58\%~$\pm$~0.21\%, a
\textbf{79.0\% relative reduction} over LBRO at 2.76\% and 88.9\% over
Pred-LB at 5.24\%, the weakest baseline. Latency is reported over
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{fig1_training_convergence.png}}
...
DRL-LSTM-LBRO reduces task drops by \textbf{26.6\%} from 0.79\% to 0.58\%
and latency by \textbf{29.5\%} from 0.369\,s to 0.260\,s relative to PPO.
...
\begin{table}[htbp]
\centering
\caption{Scalability: 6-Cloudlet Topology (3 seeds $\times$ 30 ep.,
state $\in\mathbb{R}^{41}$, 7 actions).}
\label{tab:scale}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\textbf{Algorithm} & \textbf{Drop\,\%}$\downarrow$ &
\textbf{Lat.\,(s)}$\downarrow$ & \textbf{Thpt}$\uparrow$ &
\textbf{JFI}$\uparrow$ \\
\midrule
\textbf{DRL-LSTM-LBRO} & \textbf{0.77$\pm$0.53} & \textbf{0.255} & \textbf{198.5} & 0.676 \\
...
DRL-LSTM-LBRO achieves the lowest drop rate of 0.77\% and latency
of 0.255\,s at 6 cloudlets, representing a \textbf{37.4\%} drop reduction
...
While working on this framework, several limitations were encountered
that are worth acknowledging. All experiments are conducted within a
custom Python-based M/M/$c$ discrete-event simulator, meaning the
results may not transfer directly to physical deployments where
wireless channel variability, OS scheduling jitter, and hardware
heterogeneity introduce additional sources of noise. The current
state vector has a fixed dimension tied to the number of cloudlets,
so scaling to a new topology requires retraining both the DDQN and
the LSTM predictors from scratch. Training is also computationally
modest but not negligible, requiring approximately 15--20 minutes on
an Apple M-series CPU for 1{,}000 episodes of 200 steps each,
which may limit rapid deployment iteration. Additionally, while
results are reported across ten random seeds to provide variance
estimates, formal statistical significance tests against each
individual baseline are left for future work.

Looking ahead, the most immediate direction is validating DRL-LSTM-LBRO
on a physical MEC testbed to assess how simulation-derived policies
transfer to real hardware. Incorporating realistic wireless channel
models for transmission latency estimation would make the system model
more accurate for over-the-air deployments. On the architectural side,
replacing the fixed-size state vector with a graph neural network
or attention-based encoding would allow the framework to scale
dynamically to any number of cloudlets without retraining. Finally,
extending the reward function to incorporate energy-fairness objectives
and exploring multi-agent variants where each cloudlet hosts a local
DRL agent are promising directions for further improving system
efficiency in large-scale heterogeneous MEC environments.
...
Available: https://github.com/google/cluster-data/blob/master/ClusterData2019.md,
accessed 2025.
