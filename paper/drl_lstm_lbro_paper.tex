...
over 100 episodes against five published load balancing algorithms on a
simulated three-cloudlet MEC testbed, DRL-LSTM-LBRO achieves a task drop
rate of \textbf{0.59\%} ($\pm$0.22\%, across three evaluation seeds) and
average latency of \textbf{0.34\,s}, reducing drops by \textbf{78.4\%}
and latency by \textbf{98.4\%} relative to LBRO~\cite{nayyer2022lbro}
...
\begin{enumerate}
    ...
    (0.59\% drops) and DeepRM (1.98\% drops)---an architecturally
    ...
    internalised the temporal patterns; a formally retrained
    without-LSTM variant would provide stronger evidence of this effect.
\end{enumerate}

Simulation-based evaluation against five published algorithms shows
that DRL-LSTM-LBRO achieves the lowest drop rate (0.59\%~$\pm$~0.22\%)
and latency (0.34\,s), and highest throughput (198.8 tasks/episode).
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{fig_system_model.png}}
...
for the three cloudlets. A dropped task is assigned a sentinel latency
of 999\,s in episode statistics, which correctly inflates the reported
average latency for high-drop policies.
...
\begin{figure*}[htbp]
\centerline{\includegraphics[width=0.92\textwidth]{fig_architecture1.png}}
...
produce realistic drop rates (5--15\%) under simple policies. Each policy is evaluated for 50 episodes (200 steps each) across three
random seeds (100, 42, 7); all reported metrics are means over
$3\times50 = 150$ episodes, with standard deviations computed across
seeds to provide confidence estimates. The DRL-LSTM-LBRO agent is
loaded from the best checkpoint (lowest validation drop rate) with
$\varepsilon = 0$.

\subsection{Baseline Algorithms and Ablation Variants}

We compare against five published algorithms implemented in an identical
simulation environment. One inference-time ablation variant of
DRL-LSTM-LBRO is included (same trained DDQN weights, LSTM disabled):
...
\begin{enumerate}
    ...
    \item \textbf{Ablation: no LSTM}: DRL-LSTM-LBRO with LSTM
    predictions zeroed; instantaneous EMA utilisation retained in state.
\end{enumerate}

\subsection{Results}

Table~\ref{tab:results} presents the comparative results. All metrics
are averaged over $3\times50 = 150$ episodes (three seeds, 50 episodes
each). Fig.~\ref{fig:comparison} shows a visual comparison of the four
primary metrics.

\begin{table*}[htbp]
\centering
\caption{Comparative Performance (50 episodes $\times$ 3 seeds, 200 steps/episode,
...
\textbf{JFI} $\uparrow$ \\
\midrule
\textbf{DRL-LSTM-LBRO (ours)} & \textbf{0.59 $\pm$ 0.22} & \textbf{0.9941} & \textbf{198.8} & \textbf{0.34} & 1.948 & 0.244 & 0.876 \\
\textit{Ablation: no LSTM}    & 0.59 $\pm$ 0.22          & 0.9941           & 198.8          & 0.30           & 1.946 & 0.242 & 0.869 \\
\midrule
LBRO~\cite{nayyer2022lbro}        & 2.73 $\pm$ 1.20 & 0.9727 & 194.5 & 20.63  & 1.897 & 0.246 & 0.997 \\
DeepRM~\cite{mao2016resource}     & 1.98 $\pm$ 0.88 & 0.9802 & 196.1 & 12.93  & 1.913 & 0.247 & 0.990 \\
DRL-EdgeLB~\cite{liu2022edgelb}   & 4.27 $\pm$ 1.63 & 0.9573 & 191.5 & 36.01  & 1.868 & 0.247 & 0.999 \\
MADRL-MEC~\cite{zhao2022madrl}    & 2.06 $\pm$ 1.02 & 0.9794 & 195.9 & 13.93  & 1.911 & 0.246 & 0.991 \\
Pred-LB~\cite{predlb2025}         & 5.39 $\pm$ 1.71 & 0.9461 & 189.2 & 47.06  & 1.846 & 0.243 & 0.999 \\
\bottomrule
\end{tabular}
\end{table*}
...
Index across all six algorithms.
...
DRL-LSTM-LBRO achieves the lowest drop rate (0.59\%~$\pm$~0.22\%) — a
\textbf{78.4\% relative reduction} over LBRO (2.73\%) and 89.1\% over
Pred-LB (5.39\%), the weakest baseline. Average latency drops from
20.63\,s (LBRO) to 0.34\,s; a large fraction of this 98.4\% gap is
\emph{structural}: dropped tasks are assigned a sentinel latency of
999\,s, so policies with high drop rates report heavily inflated averages.
Excluding sentinel values, the queuing delay advantage of DRL-LSTM-LBRO
remains meaningful because LSTM forecasts allow the agent to avoid
routing to cloudlets approaching saturation \emph{before} the queue
fills. Throughput is highest at 198.8 tasks/episode vs.\ 189.2 for
the weakest baseline (Pred-LB).
...
\emph{Inference-time ablation}: zeroing the LSTM predictions at
evaluation time (same DDQN weights) produces identical mean drop rate
(0.59\%~$\pm$~0.22\%) and throughput (198.8) to the full model, indicating
that the \emph{trained} policy has internalised sufficient routing knowledge
that it no longer requires LSTM predictions at inference time.
\emph{Structural comparison}: DeepRM, architecturally similar to
DRL-LSTM-LBRO without LSTM context during training, achieves
1.98\%~$\pm$~0.88\% drops — roughly $3.4\times$ worse. This gap
strongly suggests that LSTM's primary benefit manifests \emph{during
training}: richer temporal state helps the DDQN learn a more robust,
generalising policy even though the predictions are not needed at runtime.
...
\midrule
None (full model) & DRL-LSTM-LBRO          & \textbf{0.59 $\pm$ 0.22} \\
\midrule
\multirow{2}{*}{LSTM prediction}
  & Inference ablation (same DDQN) & 0.59 $\pm$ 0.22 \\
  & Structural proxy (DeepRM)      & 1.98 $\pm$ 0.88 \\
\midrule
...
\begin{itemize}
    \item A task drop rate of 0.59\%~$\pm$~0.22\% (3 seeds) — the
    lowest among all compared methods, representing a 78.4\% relative
    reduction against LBRO~\cite{nayyer2022lbro} and 89.1\% against
    Pred-LB (worst baseline).
    \item Average latency of 0.34\,s and throughput of 198.8 tasks/episode,
    both the best of all algorithms; note that a portion of the latency
    gap is structural (sentinel inflation from dropped tasks in
    high-drop baselines).
    ...
    \item Component analysis showing that LSTM's inference-time removal
    causes no additional drop rate degradation (0.59\% identical), but
    the structural comparison with DeepRM (1.98\%) confirms LSTM
    primarily benefits the \emph{training process}, with reward shaping
    as the key runtime driver.
\end{itemize}
...
\begin{itemize}
    ...
    three seeds (100, 42, 7), yielding a drop-rate standard deviation
    of $\pm$0.22\% for DRL-LSTM-LBRO. While this provides basic
    confidence estimates, a larger seed sweep ($\geq$10 seeds) and
