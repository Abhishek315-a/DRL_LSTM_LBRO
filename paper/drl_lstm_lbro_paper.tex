...
$10\times50 = 500$ episodes (ten seeds), with standard deviations
...
\begin{table}[htbp]
\centering
\caption{Scalability Results: 6-Cloudlet Topology}
\label{tab:scale}
\begin{tabular}{lcccc}
\toprule
\textbf{Algorithm} & \textbf{Drop (\%) $\downarrow$} &
\textbf{Latency (s) $\downarrow$} & \textbf{Throughput $\uparrow$} &
\textbf{JFI $\uparrow$} \\
\midrule
\textbf{DRL-LSTM-LBRO (6c)} & -- & -- & -- & -- \\
...
\textit{Note: Numbers pending 6-cloudlet training completion.}
DRL-LSTM-LBRO adapts to the expanded action space through the same
training procedure, demonstrating that the framework generalises
beyond the minimal 3-cloudlet testbed without architectural changes.
...
\begin{itemize}
    ...
    framework generalises to larger MEC topologies without
